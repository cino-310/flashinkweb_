...
export default function ToolbarInfoUpdateForm(props) {
  const {
    id: idProp,
    toolbarInfo: toolbarInfoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = toolbarInfoRecord
      ? { ...initialValues, ...toolbarInfoRecord }
      : initialValues;
    setIsVisibleInk1(cleanValues.isVisibleInk1);
    setIsVisibleInk2(cleanValues.isVisibleInk2);
    setIsVisibleInk3(cleanValues.isVisibleInk3);
    setFontSize(cleanValues.fontSize);
    setDrawingMode(cleanValues.drawingMode);
    setErrors({});
  };
  const [toolbarInfoRecord, setToolbarInfoRecord] =
    React.useState(toolbarInfoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ToolbarInfo, idProp)
        : toolbarInfoModelProp;
      setToolbarInfoRecord(record);
    };
    queryData();
  }, [idProp, toolbarInfoModelProp]);
  React.useEffect(resetStateValues, [toolbarInfoRecord]);
  const validations = {
    isVisibleInk1: [],
    isVisibleInk2: [],
    isVisibleInk3: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ToolbarInfo.copyOf(toolbarInfoRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ToolbarInfoUpdateForm")}
      {...rest}
    >
      <SwitchField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || toolbarInfoModelProp)}
